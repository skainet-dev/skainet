...
class ReshapeSlicingDslTest {
    ...
    @Test
    fun `resulting tensor from slicing has the proper Shape`() {
        // create tensor
        val original = CpuTensorFP32.fromArray(Shape(4, 5), FloatArray(20) { (it + 1).toFloat() })

        // We need to create separate views for first and last, then combine conceptually
        val tensorFromTheFirstRow = sliceTensor(original) {
            segment { range(0, 1) } // First row only
            segment { all() }       // All columns
        }
        assertEquals(Shape(1, 5), tensorFromTheFirstRow.shape)
    }
    ...
    fun testReshapeSlicingWithSegmentOperations() {
        ...
        val firstLastRows = with(backendFP32) {
            // We need to create separate views for first and last, then combine conceptually
            val firstRow = sliceTensor(original) {
                segment { range(0, 1) } // First row only
                segment { all() }       // All columns
            }
