...
class TensorDSLTest {
    ...
    private val testFactory = object : TensorDataFactory<FP32, Float> {
        ...
        override fun randn(
            ...
            MockTensorData(shape, FloatArray(shape.volume) {
                ...
                val z =
                    kotlin.math.sqrt(-2.0 * kotlin.math.ln(u1.toDouble())) * kotlin.math.cos(2.0 * kotlin.math.PI * u2.toDouble())
                (z.toFloat() * std + mean)
            })
        ...
        override fun randomInit(
            shape: Shape,
            dtype: FP32,
            generator: (random: Random) -> Float,
            random: Random
        ): TensorData<FP32, Float> =
    ...
    private class MockTensorData(
        ...
        override fun set(vararg indices: Int, value: Float) {
            data[shape.index(indices)] = value
        }
    }
    ...
    fun testBasicTensorCreation() {
        ...
        val zerosTensor = tensor<FP32, Float>(dtype)
            .shape(3, 4)
            .zeros()
            .build(testFactory)
