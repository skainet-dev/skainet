...
class SlicingTest {
    ...
    // Task 19.1: Test index mapping correctness for all slice types
    @Test
    ...
    fun testShapeCalculationForSlicedViews() {
        ...
        with(testFactory) {
            val tensor = tensor(10, 8, 6, 4) { shape ->
                init { indices ->
                    var flatIndex = 0
                    var stride = 1
                    for (i in indices.indices.reversed()) {
    ...
    fun testZeroCopyBehavior() {
        ...
        with(testFactory) {
            val tensor = tensor(1000, 1000) { shape ->
                init { indices ->
                    var flatIndex = 0
                    var stride = 1
                    for (i in indices.indices.reversed()) {
    ...
    fun testMemoryLeakPrevention() {
        ...
        with(testFactory) {
            ...
            fun createViews(): List<TensorView<FP32, Float>> {
                val tensor = tensor(100, 100) { shape ->
                    init { indices ->
                        var flatIndex = 0
                        var stride = 1
                        for (i in indices.indices.reversed()) {
    ...
    fun testPerformanceAccessPatterns() {
        ...
        with(testFactory) {
            val tensor = tensor(100, 100, 50) { shape ->
                init { indices ->
                    var flatIndex = 0
                    var stride = 1
                    for (i in indices.indices.reversed()) {
    ...
    fun testNCHWLayoutOptimizations() {
        ...
        with(testFactory) {
            ...
            val tensor = tensor(4, 3, 32, 32) { shape ->
                init { indices ->
                    var flatIndex = 0
                    var stride = 1
                    for (i in indices.indices.reversed()) {
